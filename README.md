...
- **Manual editing** – reassign any shift slot to a different employee (or leave it unfilled), with non-blocking warnings if the change violates that employee's usual constraints — including a weekly-hours overrun, too little rest before/after the shift, or (for a `windows`-mode employee) a shift outside their availability windows for that day — so HR can always override, but never by accident
...
- Individual start/end times per assignment, decoupled from the shift type's usual hours, with the shift type itself becoming optional per assignment
- Business hours and per-slot coverage requirements laid out on the actual time axis, with coverage gaps shown directly instead of only per-weekday headcounts
- Automatic trimming of a shift to what an employee's availability window actually covers, replacing today's all-or-nothing "fully contained" rule (see [Availability windows](#availability-windows) above) with real partial coverage
