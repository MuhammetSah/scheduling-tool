...
**The one feature the status report called untested really was.** `employee_availability` was empty in production — the windows mode, which is what stage 4's trimming exists for, had never been used in anger. Driven through the browser it holds up: an employee restricted to Mon–Fri 08:00–14:00, against a band demanding two people from 08:00 to 20:30, makes the planner split the band and put her on her own window while the afternoon goes to someone else. The feature was never broken, only unexercised.

**Three toggles told a screen reader nothing.** Language (DE/EN), calendar-vs-table, and the availability mode are all pairs of buttons where the selection lived purely in a CSS class — assistive technology heard two equal buttons and no indication which one applied, and the schedule page's pair had no `role`/`aria-label` tying them together either. All three now carry `aria-pressed`.
